...
<body>
    <div class="container px-10 py-5 mx-auto">
        ...
        <h2 class="text-red-500">Edit Your Info - {{ $post->name }}</h2>
        <a href="/" class="bg-green-600 text-white py-2 px-5 rounded"> Back To Home</a>


        </div>
        <div class="w-1/1 mx-auto">
            <form method="post" action="{{ route('update', $post->id) }}" enctype="multipart/form-data" class="bg-white p-5 rounded shadow-md">
            @csrf    
            <label for="">Name</label>
                <input type="text" placeholder="Name" name="name" value="{{ $post->name }}" class="border border-gray-300 rounded px-3 py-2 mb-4 w-full">
                @error('name')
                <div class="alert alert-danger bg-red-500">{{ $message }}</div>
                @enderror

                <label for="">Email</label>
                <input type="email" placeholder="Email" name="email"  value="{{  $post->email }}" class="border border-gray-300 rounded px-3 py-2 mb-4 w-full">
                 @error('email')
                 <div class="alert alert-danger bg-red-500">{{ $message }}</div>
                 @enderror
